        ...
        <transition name="slide-up">
            <div v-if="currentStep === 0" class="flex flex-col items-center pt-6 pb-20">
                <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8 w-full max-w-md text-center">
                    ...
                    <div class="space-y-6 text-left">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <select v-model="requestYear" :class="requestYear ? 'text-slate-800' : 'text-slate-500'"
                                    ...
                                    <option v-for="y in requestYears" :value="y.value" :key="y.value" class="text-slate-800">{{ y.label }}</option>
                                </select>
                                <select v-model="requestMonthPart" :class="requestMonthPart ? 'text-slate-800' : 'text-slate-500'"
                                    class="appearance-none w-full bg-slate-50 border border-slate-200 text-lg font-bold rounded-xl focus:ring-2 focus:ring-orange-500 block p-4 text-center">
                                    <option value="" disabled>月を選択</option>
                                    <option v-for="m in requestMonths" :value="m.value" :key="m.value" class="text-slate-800">{{ m.label }}</option>
                                </select>
                            </div>
                    ...
                    <button @click="startRequest" :disabled="!requestStoreKey || !requestPeriodKey"
...
<script>
import { getInventoryData, fetchLatestInventoryPeriodKey } from '../../api.js'
...
export default {
    ...
    data() {
        return {
            ...
            requestYear: '',
            requestMonthPart: '',
            requestMonths: buildMonthOptions(),
            requestYears: buildYearOptions(),
            requestItems: [],
            requestOrderState: {},
    ...
    computed: {
        ...
        requestPeriodKey() {
            return composePeriodKey(this.requestYear, this.requestMonthPart)
        }
    },
    watch: {
    ...
    methods: {
        async applyDefaultRequestMonth() {
            if (this.requestYear || this.requestMonthPart) return
            try {
                ...
                if (p) {
                    this.requestYear = String(p.year)
                    this.requestMonthPart = String(p.month)
                }
            } catch {
        ...
        async startRequest() {
            ...
            try {
                const data = await getInventoryData(this.requestPeriodKey)
