    ...
    <div id="app" class="min-h-[100dvh] flex flex-col">
        ...
        <transition name="fade">
            <div v-if="!authenticated"
                class="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-slate-900">
                <div class="w-full max-w-xs px-6">
                    ...
                    <div class="text-center mb-10">
                        ...
                        <h1 class="text-2xl font-bold text-white">店舗業務ポータル</h1>
                        <p class="text-slate-400 text-sm mt-1">暗証番号を入力してください</p>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <button v-for="n in [1,2,3,4,5,6,7,8,9]" :key="n"
                            @click="pinInput.length < 4 && (pinInput += String(n))"
                        ...
                        <div></div>
                        <!-- 0 -->
                        <button @click="pinInput.length < 4 && (pinInput += '0')"
                            class="h-16 text-2xl font-bold text-white bg-slate-700 hover:bg-slate-600 active:bg-slate-500 rounded-2xl transition-all active:scale-95 select-none">
                            0
                        </button>
                        <!-- 削除 -->
                        <button @click="pinInput = pinInput.slice(0, -1)"
...
export default {
  ...
  watch: {
    ...
    pinInput(val) { if (val.length === 4) this.checkPin() }
  },
  created() {
    if (sessionStorage.getItem('pin_auth') === 'ok') {
  ...
  methods: {
    checkPin() {
      if (this.pinInput === '4646') {
        sessionStorage.setItem('pin_auth', 'ok')
        this.authenticated = true
        this.pinError = ''
      } else {
        this.pinError = '暗証番号が違います'
        setTimeout(() => { this.pinInput = ''; this.pinError = '' }, 800)
      }
    },
