...
## 2026-05-29
- What: PIN認証のセッション持続ロジックを改善。認証済みタイムスタンプを `localStorage` に保存し、8時間TTL以内なら再読み込み後もPIN不要に。タブを明示的に閉じた場合（`pagehide.persisted=false`）はトークンを削除して次回起動時に再認証を求める
- Why: スマホの自動スリープでブラウザがタブを再読み込みするたびにPINが求められ、入力途中のデータがリセットされるケースが報告されていたため
- Files: `src/App.vue`
...
- Files: `src/components/apps/CostApp.vue`
