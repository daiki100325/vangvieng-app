...
-- 1ヶ月前（prev_period_key）の最新棚卸しIDを取得
latest_inventory as (
  select distinct on (il.store_id, il.flavor_id)
    il.id,
    il.store_id,
...
-- 2ヶ月前（prev_prev_period_key）の最新棚卸しIDを取得
...
prev_period_inventory as (
  select
    ...
    coalesce(il.tupper_basic, 0) + coalesce(il.tupper_reserve, 0)
      ...
      + coalesce(il.stock_other, 0) as base_stock
  from inventory_logs il
  join prev_latest_inventory pli
    on pli.id = il.id
),
-- 対象月の移動量（現在の在庫計算用）
period_transfer as (
...
-- 1ヶ月前（prev_period_key）の移動量（前月消費量計算用）
