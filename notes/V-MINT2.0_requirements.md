...
- **対象月のデフォルト（初期選択）** — DB に該当データがない場合はカレンダー当月／翌月でフォールバックする。
  ...
  - 原価計算: **店舗ごとに異なるため、店舗選択後に決定する**
    - 選択店舗の `cost_reports` の **最大** `period_key` の **翌月**（シーシャ済 → ドリンク月 / シーシャ未済 → 同月の継続）
    - 該当店舗の `cost_reports` がない初回は `inventory_logs` の **最大** `period_key` と同じ月（初回シーシャ用）
    - どちらもない場合は現在月
    - **自動セット完了まで年月セレクトをロックし、「対象月を自動セットしています...」を表示**。完了後はユーザーによる手動上書きを許容（過去履歴を後から入力するケース対応）
...
- 原価計算要件:
  ...
  - `シーシャ` サブモード: 集計期間（開始日・終了日）、フック本数（① 1本目 / ② チャージ / ③ おかわり / ④ スタッフ / ⑤ イベント）、ブランド別物販個数（`brands.is_cost_group=true` または `cost_group_id IS NULL` のブランドのみ）、炭種別消費量入力
    - シーシャ販売数の各項目に ①〜⑤ を付番し、UI に算式を表示する
    - シーシャ提供本数 = ① + ③ + ④ + ⑤
    - 滞在時間にかかる来店人数 = ② + ③ + ④
  - `ドリンク` サブモード: 発注日・金額・備考の随時追加・編集・削除（月内複数件可）
    - **発注日の前月期間チェック**: ドリンク発注の追加・編集時に、選択中対象月の前月の `cost_reports.start_date` 〜 `end_date`（前月シーシャ集計期間）が存在する場合、発注日がその期間内なら保存をブロックしアラートを表示する（例: 6月モードで5/20の発注を追加 → 5月のシーシャ集計期間 4/25〜5/25 内のためアラート）。シーシャモードStep5でのドリンク表示時はチェックを行わない
  ...
  - 詳細要件: [[V-MINT2.0/notes/old/V-MINT2.0_reuquirements_cost_calculation]]
- 管理者画面要件:
