...
本番配信は Cloudflare Pages で行い、既存 URL を維持したまま `V-MINT2.0` を運用している。
...
- **`AdminApp.vue`** — 4 サブモードを持つ管理者画面。入室時は `AdminPinAuth.vue` による管理者 PIN 認証を要求する。
  ...
  - `package-control`: ブランドごとの棚卸し入力で表示するパッケージサイズ（`has_pkg_50` 〜 `has_pkg_1kg`, `packages_configured`）を一括設定・保存
  - `unit-price-control`: `cost_price_masters` テーブルの価格改定履歴を管理（追加・削除）。`effective_from` 以降の月の原価計算に自動適用される
...
- **`PinAuth.vue`** — 起動時の PIN 認証オーバーレイ。認証完了まで全画面をブロック。`VITE_PIN_SHA256` / `VITE_PIN_SALT` で照合。
- **`AdminPinAuth.vue`** — 管理者画面入室時の追加 PIN 認証オーバーレイ。`VITE_ADMIN_PIN_SHA256` / `VITE_PIN_SALT` で照合。認証済みセッション中は再入力不要。
...
| `supabase/migrations/20260512_add_brand_package_flags.sql` | `brands` へのパッケージサイズフラグ列追加（`has_pkg_*` / `packages_configured`）と既存ブランドのバックフィル |
| `supabase/migrations/20260513_add_cost_price_masters.sql` | `cost_price_masters` テーブル作成と初期価格レコード挿入 |
...
- Cloudflare Pages の Production branch は `v2`
- 本番環境変数（`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_PIN_SHA256`, `VITE_PIN_SALT`, `VITE_ADMIN_PIN_SHA256`）は Cloudflare 側で管理
- ローカル検証は `.env.local` を使い、機密値はリポジトリへコミットしない
...
**Transfer App 関連（追加）:** `deleteTransferRecordBlock`（移動記録ブロック単位の削除）
